...
pub fn print_usage(program_name: &str) -> () {
    ...
    println!("Usage:");
    println!(
        "  {} <input_file>.kdn [output_file] [-O0|-O1|-O2|-O3] [-d]",
        program_name
    );
    println!(
        "  {} compile <input_file>.kdn [output_file] [-O0|-O1|-O2|-O3] [-d]",
        program_name
    );
    println!(
        "  {} run <input_file>.kdn [-O0|-O1|-O2|-O3] [-d]",
        program_name
    );
...
fn compile_file(file_path: &str, output_path: &str, opt_level: OptimizationLevel) -> Result<()> {
    ...
    eprintln!(
        "Compiling {} to {} with optimization level {:?}...",
        file_path, output_path, opt_level
    );
    ...
    if debug_mode {
        ...
        fs::write(&debug_file_path, format!("{:#?}", ast))
            .into_diagnostic()
            .map_err(|err| {
                err.context(format!("Failed to write debug info to {}", debug_file_path))
            })?;
        eprintln!("Debug information written to '{}'", debug_file_path);
    }

    eprintln!("Type checking...");
    TypeChecker::check(file_path, &ast, &source_code).map_err(miette::Report::from)?;

    eprintln!("Compiling to binary...");
    Compiler::compile(&ast, output_path, opt_level).map_err(miette::Report::from)?;
